...
<div class="row g-3 mb-4">
    <div class="col-md-3 col-sm-6">
        <div class="tile tile-green">
            <div class="tile-label">Items tracked</div>
            <div class="tile-value">{{ $totalItems }}</div>
            <div class="tile-sub">across {{ $categoryLabels->count() }} categories</div>
        </div>
    </div>
    <div class="col-md-3 col-sm-6">
        <a href="{{ route('supplies.index') }}" class="tile tile-red">
            <div class="tile-label">Needs restocking</div>
            <div class="tile-value">{{ $lowStockCount }}</div>
            <div class="tile-sub">{{ $lowStockCount === 0 ? 'All above minimum' : 'At or below minimum' }}</div>
        </a>
    </div>
    <div class="col-md-3 col-sm-6">
        <a href="{{ route('supplies.index') }}" class="tile tile-amber">
            <div class="tile-label">Expiring soon</div>
            <div class="tile-value">{{ $expiringCount }}</div>
            <div class="tile-sub">{{ $expiringCount === 0 ? 'Clear for 3 months' : 'Within 3 months' }}</div>
        </a>
    </div>
    <div class="col-md-3 col-sm-6">
        <a href="{{ route('withdrawals.index') }}" class="tile tile-blue">
            <div class="tile-label">Withdrawals</div>
            <div class="tile-value">{{ $totalWithdrawals }}</div>
            <div class="tile-sub">recorded to date</div>
        </a>
    </div>
</div>
...
(function () {
    ...
    const teal = '#0c3c7c', amber = '#eea316', red = '#790406', slate = '#5a719a';
    ...
    new Chart(document.getElementById('categoryChart'), {
        ...
        options: { plugins: { legend: { display: false } },
            scales: { y: { beginAtZero: true, ticks: { precision: 0 }, grid: { color: '#e4e9e5' }, border: { display: false } },
    ...
    new Chart(document.getElementById('trendChart'), {
        ...
        data: { labels: JSON.parse(d.dataset.trendLabels),
            datasets: [{ data: JSON.parse(d.dataset.trendCounts), borderColor: amber,
                backgroundColor: 'rgba(232,147,12,0.10)', fill: true, tension: 0.35, pointBackgroundColor: amber, pointRadius: 3 }] },
        options: { plugins: { legend: { display: false } },
            scales: { y: { beginAtZero: true, ticks: { precision: 0 }, grid: { color: '#e4e9e5' }, border: { display: false } },
                      x: { grid: { display: false } } } }
    });
